...
def number_guessing_game():
    ...
    def check_guess():
        ...
        try:
            ...
            if game_state["attempts"] >= max_attempts and guess != receive_line: # Added check to ensure it wasn't the winning guess
                result_label.config(text=f"No more attempts! The number was {receive_line}.", foreground="#F44336")  # Red
                check_button.config(state=tk.DISABLED)
                slider.config(state=tk.DISABLED)
